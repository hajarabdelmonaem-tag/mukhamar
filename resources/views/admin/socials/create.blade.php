...
@section('content')
<x.common.page-breadcrumb :title="__('admin.socials.add')" :breadcrumbs="[__('admin.socials.title') => route('admin.socials.index'), __('admin.actions.add') => null]" />
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="POST" action="{{ route('admin.socials.store') }}" enctype="multipart/form-data" class="p-6">
        @csrf
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <label class="flex cursor-pointer items-center gap-3">
                    <input type="checkbox" name="is_active" value="1" checked class="h-4 w-4 rounded border-gray-300 text-brand-500 focus:ring-brand-500 dark:border-gray-700" />
                    <span class="text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.status.active') }}</span>
                </label>
            </div>
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.socials.save') }}</button>
