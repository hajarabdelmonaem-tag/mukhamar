...
@section('content')
<x.common.page-breadcrumb :title="__('admin.payment_methods.add')" :breadcrumbs="[__('admin.payment_methods.title') => route('admin.payment_methods.index'), __('admin.actions.add') => null]" />
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="POST" action="{{ route('admin.payment_methods.store') }}" class="p-6">
        @csrf
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="name" value="{{ old('name') }}" required class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.payment_methods.save') }}</button>
