...
@section('content')
<x.common.page-breadcrumb :title="__('admin.banners.edit')" :breadcrumbs="[__('admin.banners.title') => route('admin.banners.index'), __('admin.actions.edit') => null]" />
...
@php
    $bannerImage = $banner->image
        ? (str_starts_with($banner->image, 'http') ? $banner->image : \Illuminate\Support\Facades\Storage::url($banner->image))
        : null;
@endphp
...
<div>
                    <x-form.image-upload :label="__('admin.form.upload_image')" :current-image="$bannerImage" />
                </div>
            <div>
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.banners.update') }}</button>
