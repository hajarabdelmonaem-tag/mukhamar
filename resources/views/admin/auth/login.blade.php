    ...
    <div class="relative z-1 bg-white p-6 sm:p-0 dark:bg-gray-900">
        <div class="relative flex h-screen w-full flex-col justify-center sm:p-0 lg:flex-row dark:bg-gray-900">
            <div class="flex w-full flex-1 flex-col lg:w-1/2">
                <div class="mx-auto w-full max-w-md pt-10 flex items-center justify-between">
                    <a href="/"
                    ...
                    <button type="button" data-lang-toggle
                        class="inline-flex items-center gap-2 rounded-lg border border-gray-200 bg-white px-3.5 py-2 text-sm text-gray-700 transition-colors hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-900 dark:text-gray-300 dark:hover:bg-gray-800">
                        <svg class="fill-current" width="18" height="18" viewBox="0 0 24 24" fill="none">
                            <path d="M12 2a10 10 0 100 20 10 10 0 000-20zm6.9 5.5H16a13.94 13.94 0 00-2.07-3.7A8.03 8.03 0 0118.9 7.5zM12 3.2c.95 1.24 1.9 3.16 2.2 5.3H9.8c.3-2.14 1.25-4.06 2.2-5.3zM3.1 10h3.2A18.6 18.6 0 006 12c0 .68.1 1.35.3 2H3.1a8.03 8.03 0 010-4zm1.95 7.5H8a13.94 13.94 0 002.07 3.7A8.03 8.03 0 015.05 17.5zM10 20.8c-.96-1.24-1.92-3.16-2.2-5.3h4.4c-.28 2.14-1.24 4.06-2.2 5.3zM12 12c0-.68-.1-1.35-.3-2h.6c-.2.65-.3 1.32-.3 2zm4.15-3H19a8.03 8.03 0 01-5.83 4.3A13.94 13.94 0 0017 9zm-1 10.2A13.94 13.94 0 0018 15.5h3a8.03 8.03 0 01-5.85 3.7zM15.9 14c.2-.65.3-1.32.3-2 0-.68-.1-1.35-.3-2h3.9a8.03 8.03 0 010 4h-3.9z" fill="currentColor"/>
                        </svg>
                        <span class="text-sm font-medium">{{ app()->getLocale() === 'ar' ? 'English' : 'العربية' }}</span>
                    </button>
                </div>
                            ...
                            <div class="space-y-5">
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                        {{ __('admin.auth.email') }}<span class="text-error-500">*</span>
                                    </label>
                                ...
                                <div>
                                    <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">
                                        {{ __('admin.auth.password') }}<span class="text-error-500">*</span>
                                    </label>
                                    <div x-data="{ showPassword: false }" class="relative">
                                        <input :type="showPassword ? 'text' : 'password'" name="password" placeholder="{{ __('admin.auth.password') }}" required
                                            class="dark:bg-dark-900 shadow-theme-xs focus:border-brand-300 focus:ring-brand-500/10 dark:focus:border-brand-800 h-11 w-full rounded-lg border border-gray-300 bg-transparent py-2.5 pr-12 pl-4 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
                ...
                <div class="z-1 flex items-center justify-center">
                    <div class="flex max-w-xs flex-col items-center">
                        <a href="/" class="mb-4 block">
                            <img src="/images/logo/logo.svg" alt="{{ __('admin.auth.panel_title') }}" class="dark:hidden" />
                            <img src="/images/logo/logo-dark.svg" alt="{{ __('admin.auth.panel_title') }}" class="hidden dark:block" />
                        </a>
                        <p class="text-center text-gray-400 dark:text-white/60">
                            {{ __('admin.auth.panel_title') }}
                        </p>
                    </div>
                </div>
            </div>
    ...
    <script>
        document.querySelectorAll('[data-lang-toggle]').forEach((el) => {
            el.addEventListener('click', () => {
                ...
                window.location.href = '{{ route('admin.locale', ':locale') }}'.replace(':locale', current === 'ar' ? 'en' : 'ar');
            });
        });
    </script>
@endpush
