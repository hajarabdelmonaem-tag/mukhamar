...
@section('content')
<x.common.page-breadcrumb :title="__('admin.intros.edit')" :breadcrumbs="[__('admin.intros.title') => route('admin.intros.index'), __('admin.actions.edit') => null]" />
...
@php
    $introImage = $intro->image
        ? (str_starts_with($intro->image, 'http') ? $intro->image : \Illuminate\Support\Facades\Storage::url($intro->image))
        : null;
@endphp
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="POST" action="{{ route('admin.intros.update', $intro) }}" enctype="multipart/form-data" class="p-6">
        @csrf
        @method('PATCH')
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            <div>
                ...
                <input type="text" name="title[en]" value="{{ old('title.en', $intro->getTranslation('title', 'en', false)) }}" required class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                ...
                <input type="text" name="title[ar]" value="{{ old('title.ar', $intro->getTranslation('title', 'ar', false)) }}" required dir="rtl" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div class="sm:col-span-2">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.form.description_en') }}</label>
                <textarea name="description[en]" rows="3" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description.en', $intro->getTranslation('description', 'en', false)) }}</textarea>
            </div>
            <div class="sm:col-span-2">
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.form.description_ar') }}</label>
                <textarea name="description[ar]" rows="3" dir="rtl" class="dark:bg-dark-900 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30">{{ old('description.ar', $intro->getTranslation('description', 'ar', false)) }}</textarea>
            </div>
            <div>
                <x-form.image-upload :label="__('admin.form.upload_image')" :current-image="$introImage" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.categories.sort_order') }}</label>
                <input type="number" name="sort_order" value="{{ old('sort_order', $intro->sort_order) }}" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div class="flex items-center gap-3 sm:col-span-2">
                <div x-data="{ isActive: {{ old('is_active', $intro->is_active) ? 'true' : 'false' }} }">
                    <label class="flex cursor-pointer items-center select-none">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $intro->is_active) ? 'checked' : '' }} @change="isActive = $event.target.checked" class="sr-only" />
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.intros.update') }}</button>
