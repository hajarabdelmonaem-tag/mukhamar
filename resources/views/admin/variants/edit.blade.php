...
@section('content')
<x.common.page-breadcrumb :title="__('admin.variants.edit')" :breadcrumbs="[__('admin.variants.title') => route('admin.variants.index'), __('admin.actions.edit') => null]" />
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    ...
    <form method="POST" action="{{ route('admin.variants.update', $variant) }}" class="p-6">
        @csrf
        @method('PATCH')
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div>
                ...
                <input type="text" name="name[ar]" value="{{ old('name.ar', $variant->getTranslation('name', 'ar')) }}" dir="rtl" required class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.variants.unit_en') }}</label>
                <input type="text" name="unit[en]" value="{{ old('unit.en', $variant->getTranslation('unit', 'en')) }}" placeholder="{{ __('admin.variants.unit_placeholder') }}" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.variants.unit_ar') }}</label>
                <input type="text" name="unit[ar]" value="{{ old('unit.ar', $variant->getTranslation('unit', 'ar')) }}" dir="rtl" placeholder="{{ __('admin.variants.unit_placeholder') }}" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.table.price_adjustment') }}</label>
                <input type="number" name="price_adjustment" value="{{ old('price_adjustment', $variant->price_adjustment) }}" step="0.01" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.table.sku') }}</label>
                <input type="text" name="sku" value="{{ old('sku', $variant->sku) }}" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div>
                <label class="mb-1.5 block text-sm font-medium text-gray-700 dark:text-gray-400">{{ __('admin.table.stock') }}</label>
                <input type="number" name="stock" value="{{ old('stock', $variant->stock) }}" min="0" class="dark:bg-dark-900 h-11 w-full rounded-lg border border-gray-300 bg-transparent px-4 py-2.5 text-sm text-gray-800 placeholder:text-gray-400 focus:ring-3 focus:outline-hidden dark:border-gray-700 dark:bg-gray-900 dark:text-white/90 dark:placeholder:text-white/30" />
            </div>
            <div class="sm:col-span-2 flex flex-wrap items-center gap-6">
                <div x-data="{ isDefault: {{ old('is_default', $variant->is_default) ? 'true' : 'false' }} }">
                    <label class="flex cursor-pointer items-center select-none">
                        <input type="checkbox" name="is_default" value="1" {{ old('is_default', $variant->is_default) ? 'checked' : '' }} @change="isDefault = $event.target.checked" class="sr-only" />
                ...
                <div x-data="{ isActive: {{ old('is_active', $variant->is_active) ? 'true' : 'false' }} }">
                    <label class="flex cursor-pointer items-center select-none">
                        <input type="checkbox" name="is_active" value="1" {{ old('is_active', $variant->is_active) ? 'checked' : '' }} @change="isActive = $event.target.checked" class="sr-only" />
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.variants.update') }}</button>
