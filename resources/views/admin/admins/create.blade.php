...
@section('content')
<x-common.page-breadcrumb :title="__('admin.admins.add')" :breadcrumbs="[__('admin.admins.title') => route('admin.admins.index'), __('admin.admins.add') => null]" />
    ...
    <form method="POST" action="{{ route('admin.admins.store') }}" class="p-6">
        ...
        <div class="grid grid-cols-1 gap-5 sm:grid-cols-2">
            ...
            <div class="flex items-center gap-3 sm:col-span-2">
                <button type="submit" class="inline-flex items-center justify-center rounded-lg bg-brand-500 px-6 py-3 text-sm font-medium text-white transition hover:bg-brand-600">{{ __('admin.admins.add') }}</button>
