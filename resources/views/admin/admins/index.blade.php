...
@section('content')
<x-common.page-breadcrumb title="{{ __('admin.admins.title') }}" :breadcrumbs="[__('admin.admins.title') => null]" />
...
<div class="rounded-2xl border border-gray-200 bg-white dark:border-gray-800 dark:bg-gray-900">
    <div class="flex items-center justify-between border-b border-gray-200 px-6 py-4 dark:border-gray-800">
        <h3 class="font-semibold text-gray-800 dark:text-white">{{ __('admin.admins.admin_users') }}</h3>
        <a href="{{ route('admin.admins.create') }}" class="inline-flex items-center gap-2 rounded-lg bg-brand-500 px-4 py-2.5 text-sm font-medium text-white transition hover:bg-brand-600">
        ...
        <table class="w-full text-start text-sm">
            <thead>
                <tr class="border-b border-gray-200 text-gray-500 dark:border-gray-800 dark:text-gray-400">
                    ...
                    <th class="px-6 py-3 font-medium">{{ __('admin.table.email') }}</th>
            ...
            <tbody>
                    ...
                    <tr class="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800/50">
                        ...
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <a href="{{ route('admin.admins.edit', $user) }}" class="inline-flex items-center justify-center rounded-lg border border-gray-200 p-2 text-gray-500 transition hover:bg-brand-50 hover:text-brand-500 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-brand-500/10" title="{{ __('admin.actions.edit') }}">
                                ...
                                <form method="POST" action="{{ route('admin.admins.destroy', $user) }}" onsubmit="return confirm('{{ __('admin.confirm.remove_admin') }}')">
                                    @csrf
                                    @method('DELETE')
                                    <button type="submit" class="inline-flex items-center justify-center rounded-lg border border-gray-200 p-2 text-gray-500 transition hover:bg-error-50 hover:text-error-500 dark:border-gray-800 dark:text-gray-400 dark:hover:bg-error-500/10" title="{{ __('admin.admins.remove_role') }}">
                ...
                @empty
                    <tr><td colspan="4" class="px-6 py-8 text-center text-gray-500 dark:text-gray-400">{{ __('admin.empty.admins') }}</td></tr>
                @endforelse
            </tbody>
        </table>
    </div>
